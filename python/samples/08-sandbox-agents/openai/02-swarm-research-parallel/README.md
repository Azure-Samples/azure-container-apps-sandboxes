...
```bash
cd python/samples/08-sandbox-agents/openai/02-swarm-research-parallel
```
...
uv run --extra agents swarm_research_agent.py --topic "azure-functions|Azure Functions|https://github.com/Azure/azure-functions-host|What is Azure Functions and how does it work?" --topic "firecracker|Firecracker microVMs|https://github.com/firecracker-microvm/firecracker|How does Firecracker provide lightweight isolation?"

# Research 2 web frameworks
uv run --extra agents swarm_research_agent.py --topic "fastapi|FastAPI|https://github.com/fastapi/fastapi|What makes FastAPI performant?" --topic "django|Django|https://github.com/django/django|What are Django's key features?"

# Compare 3 container runtimes (with lower concurrency)
uv run --extra agents swarm_research_agent.py --topic "kubernetes|Kubernetes|https://github.com/kubernetes/kubernetes|How does the scheduler work?" --topic "docker|Docker|https://github.com/docker/cli|What are Docker's isolation mechanisms?" --topic "containerd|containerd|https://github.com/containerd/containerd|What are containerd's core responsibilities?" --concurrency 2
```

### 4. View results

Open `.run-output/swarm-<run-id>/final-answer.md`, or browse the
`.run-output/swarm-<run-id>/` directory.
...
```bash
# Research sandbox implementations (recommended first test)
uv run --extra agents swarm_research_agent.py \
...
uv run --extra agents swarm_research_agent.py --topic "pytorch|PyTorch|https://github.com/pytorch/pytorch|What is PyTorch's autograd system?" --topic "tensorflow|TensorFlow|https://github.com/tensorflow/tensorflow|What are TensorFlow's core components?" --concurrency 2

# Research orchestration platforms
uv run --extra agents swarm_research_agent.py --topic "airflow|Apache Airflow|https://github.com/apache/airflow|What are Airflow's key concepts?" --topic "prefect|Prefect|https://github.com/PrefectHQ/prefect|How does Prefect handle workflow orchestration?" --topic "temporal|Temporal|https://github.com/temporalio/temporal|What is Temporal's architecture?"

# Analyze API frameworks
uv run --extra agents swarm_research_agent.py --topic "express|Express.js|https://github.com/expressjs/express|What is Express's middleware system?" --topic "fastify|Fastify|https://github.com/fastify/fastify|What makes Fastify fast?" --concurrency 2
```
